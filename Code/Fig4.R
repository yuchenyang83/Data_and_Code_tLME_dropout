...
#   Date        :    07.08.2025
#   Purpose     :    produce Figure 4 for Simulation
#
...
#
#   Output data files :  Data_and_Code/results/Figure4.pdf
#
#   R Version   :    R-4.3.1                                                              
...
colMeans(MNAR.fit2)
value <- c(
  apply((t(MAR.fit1[, 1:8]) - realt[1:8])^2, 1, sum) / dim(MAR.fit1)[1],
...
beta_labels <- c(
  expression(paste(beta[0])), expression(paste(beta[1])), expression(paste(beta[2])), expression(paste(beta[3])),
...
ppc <- data.frame(
  ...
  beta = factor(c(
    as.character(rep(gl(8, 1, labels = beta_labels), 10)),
...
ppc1.1$beta <- factor(ppc1.1$beta, levels = c(
  "paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
  "paste(sigma^2)",
...
ppc3.1$beta <- factor(ppc3.1$beta, levels = c(
  "paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
  "paste(sigma^2)",
  "paste(d[11])", "paste(d[12])", "paste(d[21])", "paste(d[22])",
  "paste(alpha[0][0])", "paste(alpha[0][1])", "paste(alpha[1])", "paste(alpha[2])"
...
colMeans(MNAR.est2)
value <- c(
  apply((t(MAR.est1[, 1:9]) - realt[1:9])^2, 1, sum) / dim(MAR.est1)[1],
...
ppc <- data.frame(
  ...
  beta = factor(c(
    as.character(rep(gl(9, 1, labels = beta_labels), 10)),
...
ppcTotal$beta <- factor(ppcTotal$beta, c(
  "paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
  "paste(sigma^2)",
  "paste(d[11])", "paste(d[12])", "paste(d[21])", "paste(d[22])",
...
pdf(paste0(PATH, "/Result/Figure4.pdf"), width = 17, height = 17)
multiplot(plotlist = list(RB.sim1, RB.sim2, RB.sim3), layout = layout)
dev.off()
