...
MAR.fit1 = as.matrix(read.table(paste(PATH1,'SIM1/MAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MCAR.fit1 = as.matrix(read.table(paste(PATH1,'SIM1/MCAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MNAR.fit1 = as.matrix(read.table(paste(PATH1,'SIM1/MNAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MAR.fit2 = as.matrix(read.table(paste(PATH1,'SIM2/MAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MCAR.fit2 = as.matrix(read.table(paste(PATH1,'SIM2/MCAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MNAR.fit2 = as.matrix(read.table(paste(PATH1,'SIM2/MNAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MAR.fit3 = as.matrix(read.table(paste(PATH1,'SIM3/MAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MCAR.fit3 = as.matrix(read.table(paste(PATH1,'SIM3/MCAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MNAR.fit3 = as.matrix(read.table(paste(PATH1,'SIM3/MNAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MAR.fit4 = as.matrix(read.table(paste(PATH1,'SIM4/MAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MCAR.fit4 = as.matrix(read.table(paste(PATH1,'SIM4/MCAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MNAR.fit4 = as.matrix(read.table(paste(PATH1,'SIM4/MNAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MAR.fit5 = as.matrix(read.table(paste(PATH1,'SIM5/MAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MCAR.fit5 = as.matrix(read.table(paste(PATH1,'SIM5/MCAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]
MNAR.fit5 = as.matrix(read.table(paste(PATH1,'SIM5/MNAR.para.fit.txt',sep=""),na.strings="NA",sep=""))[,-1]

names(realpara) = c("Beta0","Beta1","Beta2","Beta3", "sigma", "d11", "d12","d22", "nu", "alpha00", "alpha01","alpha1","alpha2")
colnames(MNAR.fit1) = c("Beta0","Beta1","Beta2","Beta3", "sigma", "d11", "d12","d22", "alpha00", "alpha01","alpha1","alpha2")

realt = realpara[-9]
colMeans(MCAR.fit2)
colMeans(MAR.fit2)
colMeans(MNAR.fit2)
value = c(apply((t(MAR.fit1[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MAR.fit1)[1],
          apply((t(MAR.fit2[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MAR.fit2)[1],
          apply((t(MAR.fit3[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MAR.fit3)[1],
          apply((t(MAR.fit4[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MAR.fit4)[1],
          apply((t(MAR.fit5[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MAR.fit5)[1],
          apply((t(MCAR.fit1[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MCAR.fit1)[1],
          apply((t(MCAR.fit2[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MCAR.fit2)[1],
          apply((t(MCAR.fit3[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MCAR.fit3)[1],
          apply((t(MCAR.fit4[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MCAR.fit4)[1],
          apply((t(MCAR.fit5[,1:8]) - realt[1:8])^2, 1, sum)/ dim(MCAR.fit5)[1],
          apply((t(MNAR.fit1) - realt)^2, 1, sum)/ dim(MNAR.fit1)[1],
          apply((t(MNAR.fit2) - realt)^2, 1, sum)/ dim(MNAR.fit2)[1],
          apply((t(MNAR.fit3) - realt)^2, 1, sum)/ dim(MNAR.fit3)[1],
          apply((t(MNAR.fit4) - realt)^2, 1, sum)/ dim(MNAR.fit4)[1],
          apply((t(MNAR.fit5) - realt)^2, 1, sum)/ dim(MNAR.fit5)[1]
)
...
rep(gl(8, 1, labels = beta_labels), 10)
rep(gl(12, 1, labels = beta_labels2), 5)
ppc = data.frame(value = value,
                 sample = c(rep(rep(c("25", "50", "100", "200", "400"), each = 8), 2), rep(rep(c("25", "50", "100", "200", "400"), each = 12), 1)),
                 beta = factor(c(as.character(rep(gl(8, 1, labels = beta_labels), 10)),
                                 as.character(rep(gl(12, 1, labels = beta_labels2), 5)))),
                 G = c(rep(c("MAR", "MCAR"), each = 8*5), rep(c("MNAR"), each = 12*5))
)

ppc1.1 = rbind(cbind(ppc, model = "LME"))
...
ppc1.1$beta = factor(ppc1.1$beta, levels = c("paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
                                             "paste(sigma^2)",
                                             "paste(d[11])",     "paste(d[12])", "paste(d[21])", "paste(d[22])",
                                             "paste(alpha[0][0])",  "paste(alpha[0][1])", "paste(alpha[1])" , "paste(alpha[2])"))
ppc1.1$G = factor(ppc1.1$G, c("MCAR", "MAR", "MNAR"))
ppc1.1$model = factor(ppc1.1$model, levels = c("LME"))
ppc1.1$GG = c(c(rep(c("MAR normal 25%", "MCAR normal 25%"), each = 8*5), rep(c("MNAR normal 25%"), each = 12*5)))
...
ppc2.1 = rbind(cbind(ppc, model = "LME"))
...
ppc2.1$beta = factor(ppc1.1$beta, levels = c("paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
                                             ...
                                             "paste(d[11])",     "paste(d[12])", "paste(d[21])", "paste(d[22])",
                                             "paste(alpha[0][0])",  "paste(alpha[0][1])", "paste(alpha[1])"  , "paste(alpha[2])"))
ppc2.1$G = factor(ppc2.1$G, c("MCAR", "MAR", "MNAR"))
ppc2.1$model = factor(ppc2.1$model, levels = c("LME"))
ppc2.1$GG = c(c(rep(c("MAR normal 50%", "MCAR normal 50%"), each = 8*5), rep(c("MNAR normal 50%"), each = 12*5)))
...
ppc3.1 = rbind(cbind(ppc, model = "LME"))



ppc3.1$sample = factor(ppc3.1$sample, c("25", "50", "100", "200", "400"))
ppc3.1$beta = factor(ppc3.1$beta, levels = c("paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
                                             "paste(sigma^2)",
                                             "paste(d[11])",     "paste(d[12])", "paste(d[21])", "paste(d[22])",
                                             "paste(alpha[0][0])",  "paste(alpha[0][1])", "paste(alpha[1])" , "paste(alpha[2])"))
ppc3.1$G = factor(ppc3.1$G, c("MCAR", "MAR", "MNAR"))
ppc3.1$model = factor(ppc3.1$model, levels = c("LME"))
ppc3.1$GG = c(c(rep(c("MAR normal 75%", "MCAR normal 75%"), each = 8*5), rep(c("MNAR normal 75%"), each = 12*5)))
...
ppcTotal = ppcTotal[-which(ppcTotal$beta=="paste(alpha[0][0])"),]
ppcTotal = rbind(ppcTotal, ppcTotal1)

ppcTotal$model = factor(ppcTotal$model, c("LME", "tLME"))
ppcTotal$beta = factor(ppcTotal$beta, c("paste(beta[0])", "paste(beta[1])", "paste(beta[2])", "paste(beta[3])",
                          "paste(sigma^2)",
                          "paste(d[11])",     "paste(d[12])", "paste(d[21])", "paste(d[22])",
                          "paste(alpha[0][0])",  "paste(alpha[0][1])" , "paste(alpha[1])" , "paste(alpha[2])", "paste(nu)"))



RB.sim1 = ggplot(data=ppcTotal[which(ppcTotal$missing=="25% dropout"),], aes(x=sample, y=value, colour = model, group = GG)) +
  geom_line(aes(linetype=model), size=1) + geom_point(aes(shape=model), size=3) +  xlab(NULL) + ylab("MSE") +
  facet_wrap(~beta, labeller = "label_parsed",scales="free_y", ncol = 2, dir = "v") + 
  theme(legend.position=c(-5,5))+ 
  theme(text = element_text(size=20)) +
  theme(legend.key.size = unit(1, "cm"),
        legend.key.width = unit(1.5, "cm")) +
  labs(title = "25% dropout") +
  theme(plot.title = element_text(hjust = 0.5)) + 
  theme(strip.text.x = element_text(size = 20, margin = margin(b = -2, t = -2))) +
  theme(plot.margin = unit(c(0,0.3,1.8,0), "cm"))
RB.sim1

RB.sim2 = ggplot(data=ppcTotal[which(ppcTotal$missing=="50% dropout"),], aes(x=sample, y=value, colour = model, group = GG)) +
  geom_line(aes(linetype=model), size=1) + geom_point(aes(shape=model), size=3) +  xlab(NULL) + ylab("MSE") +
  facet_wrap(~beta, labeller = "label_parsed",scales="free_y", ncol = 2, dir = "v") + 
  ...
  theme(legend.key.size = unit(1, "cm"),
        legend.key.width = unit(1.5, "cm")) +
  guides(shape = guide_legend(reverse=TRUE,title=NULL),
         linetype = guide_legend(reverse=TRUE,title=NULL),
         color = guide_legend(reverse=TRUE,title=NULL)) +
  labs(title = "50% dropout") +
  ylab("") + theme(axis.text.x =  element_text(size = 15),
...
RB.sim3 = ggplot(data=ppcTotal[which(ppcTotal$missing=="75% dropout"),], aes(x=sample, y=value, colour = model, group = GG)) +
  geom_line(aes(linetype=model), size=1) + geom_point(aes(shape=model), size=3) +  xlab(NULL) + ylab(NULL) +
  facet_wrap(~beta, labeller = "label_parsed",scales="free_y", ncol = 2, dir = "v") + 
  theme(legend.position=c(-5,5))+ 
  theme(text = element_text(size=20)) +
  theme(legend.key.size = unit(1, "cm"),
...
multiplot(plotlist = list(RB.sim1, RB.sim2, RB.sim3), layout = layout)
