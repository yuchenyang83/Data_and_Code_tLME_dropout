...
#
# Model fitting is performed using the tLME model under the MNAR mechanism
...
source(paste0(PATH, "/function/run_sensitivity_analysis.r"))
alpha <- as.numeric(prediction_ym(Data, fm2, X, Z, "ARp")$alpha.hat)
Data$yc <- prediction_ym(Data, fm2, X, Z, "ARp")$yc


##### run MNAR
source(paste0(PATH, "/function/LMMmissingSEM_fixAlpha.R"))
source(paste0(PATH, "/function/tLMMmissingSEM_fixAlpha.R"))
############################################
###########  ARp
set.seed(1)
alpha.c <- c(0.001, 0.01, 0.05, 0.1, 0.5, 1, 2, 4, 8, 16, -0.001, -0.01, -0.05, -0.1, -0.5, -1, -2, -4, -8, -16)

result <- NULL
for (l in 1:length(alpha.c)) {
  alpha[4] <- alpha.c[l]
  ...
  result <- rbind(result, cbind(t(est.MNAR.ARp$IM$out[, 1:6]), alpha[4]))
}

result.output <- data.frame(
  est.beta = result[, 1],
  est.upper = result[, 1] + 1.96 * result[, 2],
  est.lower = result[, 1] - 1.96 * result[, 2],
  alpha = result[, 3]
)
rownames(result.output) <- 1:(dim(result.output)[1])

write.table(x = result.output, (paste0(PATH, "/Data/fixed_alpha.txt")), sep = " ", row.names = T, col.names = T)
