...
est.MNAR.pvi <- compute.pvi(Data, X, Z, V,
  g = g, init.para = est.MNAR.ARp$para.est, fit = est.MNAR.ARp,
  cor.type = c("ARp"), M = 100, M.LL = 1000, P = 1, tol = 1e-6,
  max.iter = max.iter, per = 1, mechanism = c("MNAR")
)

est.MAR.pvi <- compute.pvi(Data, X, Z, V,
  g = g, init.para = est.MAR.ARp$para.est, fit = est.MAR.ARp,
  cor.type = c("ARp"), M = 100, M.LL = 1000, P = 1, tol = 1e-6,
  max.iter = max.iter, per = 1, mechanism = c("MAR")
)

est.MCAR.pvi <- compute.pvi(Data, X, Z, V,
  g = g, init.para = est.MCAR.ARp$para.est, fit = est.MCAR.ARp,
  cor.type = c("ARp"), M = 100, M.LL = 1000, P = 1, tol = 1e-6,
