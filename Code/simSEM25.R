...
#
#   Filename    :    simSEM25.R
#   Project     :    BiomJ article "Supplement: Multivariate linear mixed models
#                    with censored and non-ignorable missing outcomes,
...
source(paste0(PATH, "simulate_dropout_study.r"))
source(paste0(PATH, "tLMMmissingSEM.r"))
source(paste0(PATH, "LMMmissingSEM.r"))

# SIM1:
n <- 25
PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM1/")
# SIM2:n=50; PATH1 = paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM2/")
# SIM3:n=100; PATH1 = paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM3/")
# SIM4:n=200; PATH1 = paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM4/")
# SIM5:n=400; PATH1 = paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM5/")
...
k1 <- ggplot() +
  ...
  geom_line(data = Data, aes(x = Time, y = Var1, col = as.factor(treat), grop = as.factor(Subject))) +
  xlab("Time") +
  ylab(NULL) +
...
for (Rep in 1:Repp) {
  for (ll in 1:length(N)) {
    n <- N[ll]
    if (n == 25) PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM1/")
    if (n == 50) PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM2/")
    if (n == 100) PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM3/")
    if (n == 200) PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM4/")
    if (n == 400) PATH1 <- paste0(getwd(), "/Data/Simulation/SS-simulationSEM-t25/SIM5/")
    ...
    est.MNAR <- try(tLMM.miss.SEM(Data, X, Z, V, g, init.para,
      cor.type = c("UNC"), M = 1000, M.LL = 1000, P = 1, tol = 1e-6, max.iter = max.iter, per = 100,
      mechanism = "MNAR"
    ), silent = F)
