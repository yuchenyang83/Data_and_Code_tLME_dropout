...
fit.MNAR <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MNAR"
)

#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = alpha[-length(alpha)])
fit.MAR <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
fit.MCAR <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
# init.para = fit.MNAR$para.est
# init.para = list(Beta=fit.MNAR$para.est$Beta, DD=fit.MNAR$para.est$D, sigma=fit.MNAR$para.est$sigma, Phi=Phi, alpha=fit.MNAR$para.est$alpha)
fit.MNAR.ARp <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MNAR"
)

#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = alpha[-length(alpha)])
fit.MAR.ARp <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
#### run MCAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = c(-5.3))
fit.MCAR.ARp <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
mechanism <- "MNAR"
cor.type <- c("BAND1")
fit.MNAR.BAND1 <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = 100, per = per,
  mechanism = "MNAR"
)

#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = 0, ga = ga, alpha = alpha[-length(alpha)])
fit.MAR.BAND1 <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
fit.MCAR.BAND1 <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
mechanism <- "MNAR"
cor.type <- c("CS")
fit.MNAR.CS <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = 200, per = per,
  mechanism = "MNAR"
)
set.seed(20240101)
#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = 0, ga = ga, alpha = alpha[-length(alpha)])
fit.MAR.CS <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
# fit.MAR.treat = LMM.AECM.miss(Data, X, Z, V, g, init.para, cor.type = c("UNC"), M = 1000, P=1, tol = 1e-2, max.iter=max.iter, per=per)
set.seed(20240101)
#### run MCAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = 0, ga = ga, alpha = c(-2.15), nu = 20)
fit.MCAR.CS <- LMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = alpha, nu = 20)
mechanism <- "MNAR"
est.MNAR <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = 100, per = per,
  mechanism = "MNAR"
)

#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = alpha[-length(alpha)], nu = 20)
est.MAR <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = 100, per = per,
  mechanism = "MAR"
)
...
est.MCAR <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("UNC"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
mechanism <- "MNAR"
cor.type <- "ARp"
est.MNAR.ARp <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MNAR"
)
...
mechanism <- "MNAR"
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = alpha[-length(alpha)], nu = 20)
est.MAR.ARp <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
#### run MCAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = Phi, ga = ga, alpha = c(-5.3), nu = 20)
est.MCAR.ARp <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("ARp"), M = 1000, M.LL = 1000, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
mechanism <- "MNAR"
cor.type <- c("BAND1")
est.MNAR.BAND1 <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MNAR"
)
...
est.MAR.BAND1 <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
est.MCAR.BAND1 <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("BAND1"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
...
mechanism <- "MNAR"
cor.type <- c("CS")
est.MNAR.CS <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MNAR"
)
set.seed(20240101)
#### run MAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = 0.2, ga = ga, alpha = alpha[-length(alpha)], nu = 20)
est.MAR.CS <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MAR"
)
...
#### run MCAR
init.para <- list(Beta = Beta, DD = DD, sigma = sigma, Phi = 0.2, ga = ga, alpha = c(-2.15), nu = 20)
est.MCAR.CS <- tLMM.miss.SEM(Data, X, Z, V, g, init.para,
  cor.type = c("CS"), M = 1000, M.LL = M.LL, P = 1, tol = tol, max.iter = max.iter, per = per,
  mechanism = "MCAR"
)
