...
#   Date        :    07.08.2025
#   Purpose     :    produce Figure 3 for Simulation
#
...
#
#   Output data files :  Data_and_Code/results/Figure3.pdf
#
#   R Version   :    R-4.3.1                                                              
...
mse.sim2 <- ggplot(data = ppcTotal, aes(x = sample, y = value, colour = missing, group = missing)) +
  ...
  geom_point(aes(shape = missing), size = 4) +
  xlab("sample size N") +
  ylab("MSE") +
...
pdf(paste0(PATH, "/Result/Figure3.pdf"), width = 12, height = 8)
multiplot(plotlist = list(mse.sim2), layout = layout)
dev.off()
