################################################################################
#                                                                                     
#   Filename    :    tLMMmissingSEM_fixAlpha.R  												  
...
#
################################################################################ 
#' Fit a tLME model with non-ignorable dropout via a selection model (MCAR/MAR/MNAR)
#'
#' Fits a Student-\eqn{t} linear mixed-effects (tLME) model for longitudinal
#' outcomes jointly with a selection-model dropout mechanism under MCAR, MAR,
#' or MNAR. The longitudinal covariance combines random-effect variation and a
...
#' mechanism. Heavy tails are captured by the degrees of freedom \eqn{\nu}.
#'
...
#'   covariates used by the dropout design (e.g., \code{prevOI}). If available,
#'   \code{y.c} may hold completed values used for reporting MSE/MAE/MAPE.
...
#'     \item \code{Phi} (numeric): within-subject correlation parameter(s).
#'     \item \code{nu} (numeric): degrees of freedom for the t model.
#'   }
#' @param cor.type character(1). Working within-subject correlation structure.
#'   One of \code{"UNC"}, \code{"ARp"}, \code{"BAND1"}, \code{"CS"}.
#'   (Additional structures such as \code{"CAR1"} or \code{"DEC"} are referenced
#'   in the code path if implemented in helper functions.)
...
#' @param tol numeric(1). Convergence tolerance for the SEM loop. Default \code{1e-6}.
...
#'
#' @details
#' For subject \eqn{i} with design matrices \eqn{X_i} and \eqn{Z_i}, the
#' longitudinal covariance is \eqn{Z_i D Z_i^\top + \sigma C_i(\Phi)}, where
#' \eqn{C_i(\Phi)} depends on \code{cor.type}. The observation model is
#' multivariate \eqn{t_\nu}. Missing responses are imputed via an MCMC kernel
#' within a stochastic EM loop; parameters \eqn{(\beta, D, \sigma, \Phi, \nu, \alpha)}
#' are updated by conditional maximization. The dropout probability follows a
#' logistic link with a per-row design \eqn{V} built from \code{Data} (including
#' lagged responses under MNAR). Requires \pkg{mvtnorm}.
#'
...
#'     \code{Phi} (correlation parameters), \code{nu} (degrees of freedom),
...
#'   \item{IM2}{List. Output from \code{I.tlmm.missing()} (observed/Fisher information estimators).}
#'   \item{MSE.y}{Numeric. \eqn{\sum ( \hat y - \mathrm{Data}\$y.c )^2}.}
#'   \item{MAE.y}{Numeric. \eqn{\sum | \hat y - \mathrm{Data}\$y.c |} divided by \eqn{\sum \mathrm{Data}\$R}.}
#'   \item{MAPE.y}{Numeric. \eqn{\sum | (\hat y - \mathrm{Data}\$y.c)/\mathrm{Data}\$y.c |} divided by \eqn{\sum \mathrm{Data}\$R}.}
...
tLMM.miss.SEM <- function(Data, X, Z, V, g = g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P = 1, tol = 1e-6, max.iter = max.iter, per = 1, mechanism = c("MNAR", "MAR", "MCAR")) {
  ...
  for (i in 1:N)
  {
    ...
    if (sum(i == y.na.ind) == 0) {
      ...
      for (jj in 1:(M.LL / 100))
      {
        ...
        fn <- mvtnorm::dmvt(y.hat.i, X[idx1, ] %*% Beta, TLam[idx1, idx1], df = nu, log = F) * prod(prod((1 - pvi)^(1 - R[idx1]) * pvi^(R[idx1])))
  ...
  repeat  {
    ...
    for (m in 2:M)
    {
      for (i in y.na.ind)
      {
        ...
        y.samp[m, idx1] <- MH.y.miss.t(
          ...
          Vi = as.matrix(V[idx1, ]), R.i = R[idx1], n.i = ni[i], mu.mo = mu.mo, Sig.mm.o = Sig.mm.o, na.idx.i = na.idx[[i]]
        )
      }
    }
    ...
    for (i in 1:N)
    {
      ...
      for (m in 1:mc.size)
      {
        ...
        if (sum(c(exp(a.i %*% alpha)) == Inf) > 0) pvi.i[which(c(exp(a.i %*% alpha)) == Inf)] <- 1
  ...
  return(list(
    model.inf = model.inf, para.est = para.est, iter = iter, y.c = y.hat, IM2 = IM2,
...
#' Observed-information (Fisher) components for the tLME–dropout selection model
#'
...
#' @param X matrix. Fixed-effects design matrix aligned with the stacked data.
...
#' @param q integer(1). Random-effects dimension (number of columns in \code{Z}).
