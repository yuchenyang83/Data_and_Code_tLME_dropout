####### Mixture of LMMs with missing data #######
LMM.miss.SEM = function(Data, X, Z, V, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR','MCAR'))
{
  ...
  for(i in 1: N){
    if(i == 1) idx0 = 1: cumsum.ni.o[1]
    else idx0 = (cumsum.ni.o[i-1]+1): cumsum.ni.o[i]
  ...
  # which((colMeans(y.samp) - Data$y.c)==0)
  # c(1:n)[-na.ind]
  # all.equal(which((colMeans(y.samp) - Data$y.c)==0), c(1:n)[-na.ind])
  burn.in = M/2
  cho = seq(1, burn.in, 5)
  mc.size = length(cho)
  ...
  #### log-likelihood 1
  # loglik.MC = numeric(mc.size)
  # for(m in 1: mc.size){
  #   sum.log.fv = 0
  #   V = V.fun(y.conv[m,], Data.miss)
  #   pvi = c(exp(V %*% alpha) / (1 + exp(V %*% alpha)))
  #   # sum.log.fv = log(prod(pvi^R) * prod((1-pvi)^(1-R)))
  #   if(prod(pvi^R) * prod((1-pvi)^(1-R)) == 0) sum.log.fv = 0
  #   if(prod(pvi^R) * prod((1-pvi)^(1-R)) == 0) cat("sum.log.fv = 0 ", "\n")
  #   else sum.log.fv = log(prod(pvi^R) * prod((1-pvi)^(1-R)))
  #   
  #   # Y.cent = y.conv[m, ] - Xbeta
  #   sum.log.fy = mvtnorm::dmvnorm(y.conv[m,1:cumsum.ni[1]], X[1:cumsum.ni[1],]%*%Beta, TLam[1:cumsum.ni[1], 1:cumsum.ni[1]], log=T)
  #   for(i in 2:N) sum.log.fy = sum.log.fy + mvtnorm::dmvnorm(y.conv[m, (cumsum.ni[i-1]+1): cumsum.ni[i]], X[(cumsum.ni[i-1]+1): cumsum.ni[i], ]%*%Beta, TLam[(cumsum.ni[i-1]+1): cumsum.ni[i], (cumsum.ni[i-1]+1): cumsum.ni[i]], log=T)
  #   loglik.MC[m] = sum.log.fv + sum.log.fy
  # }
  # loglik.old1 = mean(loglik.MC)
  ...
  for(i in 1:N)
  {
    ...
    if(sum(i==y.na.ind)==0){
      ...
      for(jj in 1:(M.LL/100))
      {
        ...
        fn = mvtnorm::dmvnorm(y.hat.i, X[idx1, ]%*%Beta, TLam[idx1,idx1], log=F) * prod(prod((1-pvi)^(1-R[idx1]) * pvi^(R[idx1])))
  ...
  cat("Mixture of Linear mixed modles with ", cor.type[1], " errors: ", "\n")
  ...
  repeat
  {
    ...
    ### test variance matrix of b
    # solve(1/sigma * t(TZ) %*% TCor.inv %*% TZ + solve(TD))
    # all.equal(solve(1/sigma * t(TZ) %*% TCor.inv %*% TZ + solve(TD)), TSig.b)
    
    # ### 1
    # sum.y2 = sum.b2 = sum.yb = 0
    # sum.e2 = b= 0
    # for(m in 1: mc.size){
    #   Yhat.cent = c(y.conv[m, ] - Xbeta)
    #   sum.b2 = sum.b2 + (TD %*% t(TZ) %*% TLam.inv %*% Yhat.cent %*% t(TD %*% t(TZ) %*% TLam.inv %*% (Yhat.cent)))
    #   sum.e2 = sum.e2 + (y.conv[m, ] - Xbeta - TZ %*% (TD %*% t(TZ) %*% TLam.inv %*% Yhat.cent)) %*% t(y.conv[m, ] - Xbeta - TZ %*% (TD %*% t(TZ) %*% TLam.inv %*% Yhat.cent))
    #   
    #   yy = (y.conv[m, ] %*% t(y.conv[m, ]))
    #   sum.y2 = sum.y2 + yy
    #   sum.yb = sum.yb + ((yy - y.conv[m, ]%*%t(Xbeta)) %*% TLam.inv %*% TZ %*% TD)
    # }
    # # y.hat = y.hat/mc.size
    # y.hat = colMeans(y.conv)
    # b.hat = TD %*% t(TZ) %*% TLam.inv %*% (y.hat - Xbeta)
    # E.hat = sum.e2/mc.size # + TZ %*% TSig.b %*% t(TZ)
    ...
    # yp.hat = y.hat[-na.ind]
    # yp.cent = yp.hat - Xp %*% beta
    # mu.mp = Xm%*%beta + TLam.mp%*% TLam.pp.inv %*% yp.cent
    # Sig.mm.p = TLam.mm - TLam.mp %*% TLam.pp.inv %*% t(TLam.mp)
    # 
    # Xbeta = X %*% beta
    # for(m in 2: msize){
    #   for(i in na.subj){
    #     y.samp[m, idx[[i]]] = MH.ym(yi=y.samp[(m-1), idx[[i]]], Xbeta.i=c(Xbeta)[idx[[i]]], Lam.i=TLam[idx[[i]], idx[[i]]], alpha=alpha, Mi=Mmat[[i]], vi=v[idx[[i]]][-c(1:si[i])], ni=ni[i], Ni=si[i], mu.mp=mu.mp, Sig.mm.p=Sig.mm.p, na.idx.i=na.idx[[i]])
    #   }}
    # all.equal(yo.hat, yo, y.hat[-na.ind])
  ...
  # Model selection
  # if(mechanism=="MCAR") ma = 0
  # else ma = length(c(alpha))
  ma = length(c(alpha))
  ...
  IM = I.lmm.missing(para.est, cor.type=cor.type, N=N, cumsum.ni=cumsum.ni, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
                     y.conv=y.conv, mechanism=mechanism, dropout.idx=dropout.idx, R=R, n=n, Data.miss=Data.miss)
  IM2 = I.lmm.missing2(para.est, cor.type=cor.type, N=N, cumsum.ni=cumsum.ni, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
                     y.conv=y.conv, mechanism=mechanism, dropout.idx=dropout.idx, R=R, n=n, Data.miss=Data.miss)
  ...
  # U = wden / indv.den
  # z = t(apply(U, 1, order))-1
  return(list(model.inf = model.inf, para.est = para.est, iter=iter, y.c=y.hat, MSE.y = sum((y.hat- Data$y.c)^2),
              na.ind = na.ind, Tpara=Tpara, IM=IM, IM2=IM2))
}
...
I.lmm.missing = function(para.est, cor.type=cor.type, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, Data.miss)
{
  ...
  for(m in 1: mc.size)
  {
    ...
    SS.alpha = SS.alpha + (S.alpha %*% t(S.alpha))
  }
  H.alpha = - H.alpha/mc.size
  S.alpha.mean = S.alpha/mc.size
  SS.alpha.mean = SS.alpha/mc.size 
  ...
  # if(mechanism=='MNAR' | mechanism=='MAR'){
  I.alpha = -H.alpha - SS.alpha.mean  + S.alpha.mean%*%t(S.alpha.mean)
  V.alpha = solve(I.alpha)
  sd.alpha = sqrt(diag(V.alpha))
  out = rbind(EST, c(sd.theta, sd.alpha))
  ...
  SD = list(out=out, se = c(sd.theta, sd.alpha), I.theta=I.theta, V.theta=V.theta, I.alpha=I.alpha, V.alpha=V.alpha)
  # }
  return(SD)
}

I.lmm.missing2 = function(para.est, cor.type=cor.type, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, Data.miss)
{
  Beta = para.est$Beta
  DD = para.est$D
  sigma = para.est$sigma
