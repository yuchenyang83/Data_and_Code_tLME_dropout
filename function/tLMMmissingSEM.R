...
tLMM.miss.SEM = function(Data, X, Z, V, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR','MCAR'))
{
  begin = proc.time()[1]
  ...
  cat("t Linear mixed modles with ", cor.type[1], " errors: ", "\n")
  ...
  repeat
  {
    ...
    # y.samp = matrix(rep(Data$y.c, M), nrow=M, ncol=n, byrow=T)
  ...
  IM = I.tlmm.missing(para.est, cor.type=cor.type, X=X, N=N, cumsum.ni=cumsum.ni, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
                        y.conv=y.conv, mechanism=mechanism, dropout.idx=dropout.idx, R=R, n=n, q=q, Data.miss=Data.miss)
  IM2 = I.tlmm.missing2(para.est, cor.type=cor.type, X=X, N=N, cumsum.ni=cumsum.ni, ni=ni, mc.size=mc.size, TLam.inv=TLam.inv,
                      y.conv=y.conv, mechanism=mechanism, dropout.idx=dropout.idx, R=R, n=n, q=q, Data.miss=Data.miss)
  # Model selection
...
I.tlmm.missing = function(para.est, cor.type=cor.type, X, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, q, Data.miss)
{
  ...
  H = SS = matrix(0, ncol=(p+g+1), nrow=(p+g+1))
  S = numeric((p+g+1))
  ...
  for(m in 1: mc.size)
  {
    ...
    #   # H beta gamma
    #   H1[1:p, (p+s)] = H1[(p+s), 1:p] =  t(rep(ka2, ni) * X) %*% TLam.inv %*% ys.cent %*% t(ys.cent) %*% TLinv.dotL[[s]] %*% TLam.inv %*% ys.cent -
    #     t(rep(tau, ni) * X) %*% TLinv.dotL[[s]] %*% TLam.inv %*% ys.cent
    #   # H gamma gamma
    #   for(l in 1: s) H1[(p+s),(p+l)] = H1[(p+l),(p+s)] = sum(diag((TLinv.dotL[[l]]) %*% TLinv.dotL[[s]]))/2 + 
    #     (t(rep(ka2, ni) * ys.cent)%*% (TLinv.dotL[[l]] %*% TLam.inv %*% ys.cent %*% t(ys.cent) %*% TLinv.dotL[[s]] %*% TLam.inv %*% ys.cent)) / 2 -
    #     (t(rep(tau, ni) * ys.cent)%*% (TLinv.dotL[[l]] %*% TLinv.dotL[[s]] %*% TLam.inv + TLinv.dotL[[s]] %*% TLinv.dotL[[l]] %*% TLam.inv) %*% ys.cent) / 2
    #   # H gamma nu
    #   H1[(p+s),p+g+1] = H1[p+g+1,(p+s)] = (t(rep((ka1 - ka2), ni) * ys.cent) %*% TLinv.dotL[[s]] %*% TLam.inv %*% ys.cent) / 2
    # }
    # H.gamma = H.gamma + H1
    ...
    for(s in 1: (g1+g2+g3)){
      ...
      for(i in 1:N)
      {
        ...
        # H gamma gamma
        for(l in 1: s) H1[(p+s),(p+l)] = H1[(p+s),(p+l)] + sum(diag(TLinv.dotL[[l]][idx1,idx1] %*% TLinv.dotL[[s]][idx1,idx1]))/2 + 
    ...
    #   if(i == 1) idx1 = 1: cumsum.ni[1]
    #   else idx1 = (cumsum.ni[i-1]+1): cumsum.ni[i]
    #   kk = kk + t(rep(tau[i], ni[i]) * X[idx1,])%*%TLam.inv[idx1,idx1]%*%ys.cent[idx1,]
    # }
  ...
  SD = list(out=out, se = c(sd.theta, sd.alpha), 
            I.theta=I.theta, V.theta=V.theta, I.alpha=I.alpha, V.alpha=V.alpha)
  # }
  return(SD)
}

I.tlmm.missing2 = function(para.est, cor.type=cor.type, X, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, q, Data.miss)
{
  Beta = para.est$Beta
  DD = para.est$D
  sigma = para.est$sigma
