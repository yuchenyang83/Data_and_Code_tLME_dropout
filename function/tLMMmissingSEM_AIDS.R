################################################################################
#                                                                                     
#   Filename    :    tLMMmissingSEM_AIDS.R  												  
...
#'   \item{IM}{List. Output from \code{I.tlmm.missing()} (observed/Fisher information estimators).}
...
#'   \item{Taccept.rate}{Numeric matrix. MCMC acceptance indicators/ratios across iterations (diagnostic).}
#' }
tLMM.miss.SEM <- function(Data, X, Z, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, tol = 1e-6, max.iter = max.iter, per = 1, mechanism = c("MNAR", "MAR", "MCAR")) {
  begin <- proc.time()[1]
...
I.tlmm.missing <- function(para.est, cor.type = cor.type, X, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, q, Data.miss) {
  ...
  for (m in 1:mc.size)
  {
    if (m %% 25 == 0) cat(rep("=", 10), "Fisher information ", m / mc.size * 100, " % ", rep("=", 10), sep = "", "\n")
