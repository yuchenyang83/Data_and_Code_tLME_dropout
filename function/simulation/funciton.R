...
# R.pdf = function(wi, Ri)
# {
#   wi = c(1,wi)
#   k = length(wi)
#   R.binm = rep(NA, k)
#   for(kk in 1:k) R.binm[kk] = (wi[kk]^Ri[kk]) * ((1-wi[kk])^(1-Ri[kk]))
#   return(R.binm)
# }
...
# log.pos.y(y[idx], R[idx], w[na.idx], X[idx,], TLam[[i]][idx,idx])

# log(prod(c(1,wi)^Ri) * prod((1-c(1,wi))^(1-Ri)))
# yi = y.c[idx]
# Ri = R[idx]
# wi = w[na.idx]
# Xi = X[idx,]
# Lam.i = TLam[[i]][idx,idx]
# 
# sum(dbinom(Ri, 1, c(1,wi), log = T)[-1])
# R.pdf(wi, Ri)
# y.c = y.gen[,,2]
# Data[which(Data$Subject==166),]
# MH.y.miss(y.gen[,,m-1], R, X, Beta, y.na.ind, TLam[[i]], V, psi, TO, TM)
...
log.pos.y1 = function(yi, Ri, wi, Xi, Lam.i, Beta, mean.j, var.j)
{
  ...
  if(length(yi)==1) fy = fy - log(dnorm(yij, mean=mean.j, sd=sqrt(var.j)))
  else fy = fy - log(mvtnorm::dmvnorm(yij, mean = mean.j, sigma = var.j))
...
gen.tlmm = function(n, para, cor.type, si, q)              
{
  ...
  nu = para$nu
  Phi = para$Phi
  if(n %% 2 == 0) N = c(n/2, n/2)
  else N = rmultinom(n=1, size=n, prob=c(0.5,0.5))
  treat = rep((1:2), N)-1
  Ymat = X = Z = NULL
  for(i in 1:n)
  {
...
add.MNAR.tLMM = function(Data, Ymat, si, alpha)
{
  ...
  return(list(Ymat = Ymat, Ymat.na =  Ymat.na, Data.na = Data, Pna=p.na, D=D, 
