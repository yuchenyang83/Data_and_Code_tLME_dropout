...
tLMM.miss.SEM = function(Data, X, Z, V, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR',"MAR",'MCAR'))
{
  ...
  repeat
  {
    ...
    # for(m in 2: M)
    # {
    #   y.samp[m, ] = MH.y.miss.t.new(y.samp, Data.miss, y.na.ind, Sig.mm.o.MC, Xbeta, TLam, tau, R, alpha, ni, cumsum.ni, m)
    #   print(m)
    # }
    # 
...
# I.tlmm.missing1 = function(para.est, cor.type=cor.type, X, N, cumsum.ni, ni, mc.size, TLam.inv, y.conv, mechanism, dropout.idx, R, n, q, Data.miss)
# {
#   Beta = para.est$Beta
#   DD = para.est$D
#   sigma = para.est$sigma
