...
#' Build per-visit observed/missing indicators by time block
#'
...
#' returns \code{TRUE}.
#'
#' @param Data data.frame (or tibble). Stacked long-format data containing at
#'   least the response column \code{Var1}. Rows are ordered by visit time and
#'   aligned with \code{cumsum.nj}.
#' @param n integer. Number of distinct visit times.
#' @param nj integer vector of length \eqn{n}. The number of rows (subjects) at
#'   each visit time.
#' @param cumsum.nj integer vector of length \eqn{n}. Cumulative sums of
#'   \code{nj}, i.e., \code{cumsum.nj[j] = sum(nj[1:j])}.
#' @param N integer. Total number of rows, typically \code{sum(nj)}.
#'
#' @details
...
#' Build per-visit selection matrices for observed and missing sub-vectors
#'
...
#'
#' @param Data data.frame (or tibble). Must contain response column \code{Var1};
#'   \code{is.na(Var1)} determines missingness.
#' @param n integer. Number of visit times.
#' @param nj integer vector of length \eqn{n}. Number of rows at each visit.
#' @param cumsum.nj integer vector of length \eqn{n}. Cumulative sums of
#'   \code{nj}, i.e., \code{cumsum.nj[j] = sum(nj[1:j])}.
#' @param N integer. Total number of rows, typically \code{sum(nj)}.
#' @param R numeric or integer vector of length \eqn{N}. Per-row missingness
...
#' Blocked Metropolis–Hastings update over multiple subjects (tLME outcome)
#'
...
#'
#' @param y.samp numeric matrix. MCMC sample history of the stacked outcome
#'   vector; the current state is taken from row \code{m - 1}.
#' @param Data.miss data.frame. Stacked rows from \code{Data} used to rebuild
#'   the selection design via \code{V.fun()}.
#' @param y.na.ind integer vector. Indices of subjects to update (e.g., those
#'   with at least one missing entry).
#' @param Sig.mm.o.MC numeric matrix. Proposal covariance for the stacked
#'   missing block (used in \code{mvtnorm::rmvnorm}).
#' @param Xbeta numeric vector. Stacked mean \eqn{X \beta} for all entries.
#' @param TLam numeric matrix. Block covariance of the stacked outcome.
#' @param tau numeric vector. Subject-specific scale parameters; length equals
#'   the number of subjects.
#' @param R integer/logical vector. Response indicators on the stacked vector
#'   with convention \code{R == 1} for missing and \code{R == 0} for observed.
#' @param alpha numeric vector. Selection-model coefficients used in the joint
#'   target (via \code{yv.joint.log.t()}).
#' @param ni integer vector. Per-subject lengths \eqn{(n_1,\ldots,n_S)}.
#' @param cumsum.ni integer vector. Cumulative sums of \code{ni} used to locate
#'   subject blocks in the stacked vector.
#' @param m integer(1). Current MCMC iteration index; the previous state is
#'   \code{y.samp[m - 1, ]}.
#'
#' @details
...
#'
#' @return A numeric vector of the same length as \code{y.samp[1, ]} containing
...
#' Conditional mean imputation and selection-model fit for MNAR analysis
#'
...
#'
#' @param Data data.frame in long format with at least the columns
...
#' @param fm2 fitted mixed-effects model object whose variance components can be
#'   queried via \code{VarCorr()} and residual scale via \code{sigma}; e.g., an
#'   \code{lme4::lmer} object. The function extracts a 2×2 random-effect
#'   covariance \eqn{D} and builds subject-specific blocks \eqn{Z_i D Z_i^\top}.
#' @param X numeric matrix, fixed-effects design for all stacked rows in
#'   \code{Data} (row order must match \code{Data}); columns must be conformable
#'   with \code{fixef(fm2)}.
#' @param Z numeric matrix, random-effects design for all stacked rows (row
#'   order must match \code{Data}); the number of columns must equal \code{q}.
#' @param cor.type character scalar, correlation structure label passed to
