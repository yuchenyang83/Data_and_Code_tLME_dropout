...
LMM.miss.SEM = function(Data, X, Z, V, g=g, init.para, cor.type = c("UNC", "ARp", "BAND1", "CS"), M = 100, M.LL = 1000, P=1, tol = 1e-6, max.iter=max.iter, per=1, mechanism=c('MNAR','MCAR'))
{
  ...
  return(list(model.inf = model.inf, para.est = para.est, iter=iter, y.c=y.hat, MSE.y = sum((y.hat- Data$y.c)^2)/sum(Data$R), MAE.y = sum(abs((y.hat- Data$y.c)))/sum(Data$R), MAPE.y = sum(abs((y.hat- Data$y.c)/Data$y.c))/sum(Data$R),
